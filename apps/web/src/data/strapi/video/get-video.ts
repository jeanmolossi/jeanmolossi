...
export const cachedGetVideo = cache(getVideo);

export async function getVideo(canonicalUrl: string): Promise<Video> {
    try {
        const {
            data: { data },
        } = await strapi.get<VideosFilterResult>('/videos', {
            params: {
        ...
        const { title, videoID, excerpt: description, publishedAt } = video;
        ...
        //     "height": 720
        //   }
        const getCover = (id: string, size: string) =>
            format('https://i.ytimg.com/vi/%s/%sdefault.jpg', id, size);
